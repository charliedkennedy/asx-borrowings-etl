...
def match_targets(targets: Iterable[dict], index: list[dict]) -> tuple[list[dict],list[dict]]:
    ...
    for target in targets:
        ...
        elif top[0][2]: status,selected,confidence,reason=("AMBIGUOUS_MATCH",{},top[0][0],"multiple plausible PDFs require review") if len(top) > 1 and top[1][2] else ("MATCHED_MEDIUM",top[0][3],top[0][0],"identity likely; legal-name variation")
